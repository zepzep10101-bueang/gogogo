...
def read_root():
    ...
    <body>
        ...
        <script>
                    ...
                    ws.onmessage = async function(event) {
                        try {
                            ...
                            if (data.type === "identity_error") {
                                const key = prompt("이 닉네임은 다른 브라우저에 등록되어 있어요. 기존 브라우저의 ‘내 접속키’에서 복사한 키를 입력하세요.");
                                ws.onclose = null; ws.close();
                                if (key) { localStorage.setItem('room-key:' + window.myNickname, key.trim()); connectWebSocket(); }
            ...
            function showIdentityKey() { prompt('다른 기기에서 같은 닉네임을 사용할 때 필요한 개인 접속키입니다. 본인만 보관하세요.',identityKey()); }
